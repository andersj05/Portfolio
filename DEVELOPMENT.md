...
- protect `main` as the production branch;
- work through short-lived topic branches and pull requests;
...
Cloudflare Pages remains the hosting target. Preview deployments belong to pull
requests; production deploys only from `main` after required checks pass.

For the first release:

1. merge the approved pull request to `main`;
2. verify the production deployment;
3. connect both `andersj.dev` and `www.andersj.dev`;
4. redirect one hostname to the canonical hostname;
5. verify HTTPS, canonical metadata, and the not-found response;
6. run a production smoke test and record the release.
...
Configure the `main` branch ruleset in GitHub:
